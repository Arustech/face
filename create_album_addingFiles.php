...
<div class="container">
   ...
   <br><br>
   <?php 
   $album_obj=$main->load_model("Album");
   
   if(isset($_GET['album_id']) && $_GET['album_id']!=""){
        $album_info= $album_obj->get_album_info($_GET['album_id']); 
        $album_name=$album_info['album_name'];
         ?>  
   <span style='color: #016E88;font-family: sans-serif;font-size: 19px'> Album Name <input id='album_name' class='form-control' type='text' style='width:300px' value='<?=$album_name ?>'>
   </span>
   <input id="album_id" name="album_id"  class="form-control hidden" type="text" value="<?=$_GET['album_id']?>">
     <?php  }
               ...
               Drop Here <span style="color:white;text-transform:none;font-size: 13px">     OR    <span>
                     ...
                     <div class=" status_footer" style="display:flex;display: -ms-flexbox;" >  
                         <div class=" status_post" style="margin-right:5px">
                           <div class="btn-group">
                               <?php  if(isset($_GET['album_id']) && $_GET['album_id']!=""){ ?>
                               <button id="post_picture" style="background:#0986A3"  name="post_status" class="btn btn-primary btn_post" style="width:110px">Update Album</button>
                               <?php } else{?>
