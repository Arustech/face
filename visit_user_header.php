...
{
$visit_username  = $_GET['user_name'];    
$res    = $obj_user->getUserByUsername($visit_username);

if($res)
{
$visit_user_id  = $res['user_id'];
$basic_ = $main->load_model('BasicProfile');
$basic =$basic_->getBasicProfileByID($visit_user_id);

$contact_ = $main->load_model('ContactProfile');
$contact = $contact_->get_contact($visit_user_id);    
}else{
...
?>    
            ...
            <div class="row cover" id="cover">
             ...
             <a class=" fancybox" rel="ligthbox" data-image-id="146" title=" " href="<?= $main->config['cover_web'].$res['user_cover']?>">
                <img id="img_cover" width="970px" height="315" src="<?= $main->config['cover_web'].$res['user_cover']?>"  title="Cover Photo">
            </a>
...
<!--                    <form id="file-submit" method="POST" action="save_cover.php" enctype="multipart/form-data">
                        ...
                        <span id="change_cover"  class="btn btn-default btn-file" style="height:23px;padding:0px 5px 3px 5px;font-size: 13px;border-radius: 2px">
                           ...
                           <input type="text" name="userid" value="<?= $res['user_id']?>" hidden>   
                        </form>-->
                    </div>
            ...
            <div class="row " id="header_content">
               <div class="col-md-2  display_pic" id="dp">
                   <a class=" fancybox" rel="ligthbox" style=" text-decoration:none !important;" data-image-id="146" title=" " href="<?= $main->config['avatar_web'].$res['user_avatar']?>">
                          <img width="140px" height="140" src="<?= $main->config['avatar_web'].$res['user_avatar']?>"  title="Display Picture">
...
<span>   
               ...
               <div class="col-md-2 header_inner_content">
                  <a  href=""><?=ucfirst($res['user_name'])?></a>
           ...
           <div  class="col-xs-6 navi">
            <nav class="navigation ">
            	...
            	<ul id="menu">
                	...
                	<li ><a  href="<?= $main->config['web_path'] ?>visit_user_wall.php?user_name=<?=$visit_username?>">Wall</a></li>
